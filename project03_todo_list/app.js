...
var Operation;
(function (Operation) {
    Operation["Add_Tasks"] = "Add Tasks";
    Operation["Edit_Tasks"] = "Edit Tasks";
    Operation["Delete_Tasks"] = "Delete Tasks";
    Operation["Mark_As_Done"] = "Mark As Done";
    Operation["View_List"] = "View List";
    Operation["Exit"] = "Exit";
})(Operation || (Operation = {}));
const print = (...message) => {
    console.log(...message);
...
async function addTasksFn() {
    ...
    if (addTasks.trim() !== "") {
        if (!todoList.includes(addTasks.trim())) {
            ...
            print(chalk.bgGreenBright.bold(`The task ${chalk.whiteBright(addTasks)} was added succesfully!`));
        }
        else {
...
async function markedAsDone() {
    if (todoList.length !== 0 &&
        ...
        const { selectTasks } = await inquirer.prompt({
            name: "selectTasks",
            type: "checkbox",
        ...
        if (selectTasks.length !== 0) {
            selectTasks.forEach((task) => {
                ...
                todoList[index] += " \u2714 DONE!";
                print(chalk.bgGreenBright.bold(`The task ${chalk.whiteBright(task)} was marked done succesfully!`));
            });
        }
        else {
    ...
    if (todoList.length !== 0) {
        ...
        if (deleteTasks.length !== 0) {
            deleteTasks.forEach((task) => {
                ...
                print(chalk.bgGreenBright.bold(`The tasks ${chalk.whiteBright(deleteTasks)} was deleted succesfully!`));
            });
        }
        else {
...
async function editTasksFn() {
    if (todoList.length !== 0) {
        ...
        if (editWithTasks.trim() !== "") {
            ...
            print(chalk.bgGreenBright.bold(`The task ${chalk.whiteBright(editTasks)} was edited with ${chalk.bgBlack.whiteBright(editWithTasks)} succesfully!`));
        }
        else {
            print(chalk.yellow("Task description cannot be empty."));
        }
