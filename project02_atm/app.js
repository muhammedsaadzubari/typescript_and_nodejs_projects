...
const fastCash = async (userAccount) => {
...
const balanceInquiry = async (userAccount, words) => {
    console.log(chalk.yellow(`Your ${words} balance is: ${userAccount.Balance}`));
};
const withdrawl = async (userAccount) => {
...
const promptTypeList = async (choices, message) => {
    let { prompt } = await inquirer.prompt({
        name: "prompt",
        type: "list",
        message,
...
const SignUp = async () => {
    let operation = await promptTypeList(["Sign UP", "Exit"], "Which operation do you want to perform?");
    switch (operation) {
        case "Sign UP":
...
let LogIn = async () => {
    let operation = await promptTypeList(["Log IN", "Exit"], "Which operation do you want to perform?");
    switch (operation) {
        case "Log IN":
            let userAccount = await inquirer.prompt([
                {
                    ...
                    validate: (ID) => {
                        if (ID.match(/^\d{5}$/)) {
                            ...
                            console.log(chalk.red("Enter a valid ID! (Consists of 5 numbers)"));
                            return false;
                        }
                    },
                },
...
let atm = async (userAccount) => {
    let operation = await promptTypeList(["Balance Inquiry", "Fast Cash", "Cash Withdrawl", "Exit"], "Select operation to perform:");
    switch (operation) {
    ...
    while (bool) {
        ...
        if (signUp !== undefined && login !== undefined) {
            if (login.Email === signUp.Email &&
                ...
                let running = true;
                console.log(chalk.greenBright(`Hi ${signUp.Name}! You have logged in!`));
                while (running) {
                    await atm(signUp);
