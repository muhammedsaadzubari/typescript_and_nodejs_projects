...
async function getAmount(currency: string) {
...
async function restartOrExit(): Promise<boolean> {
...
async function convertCurrency(fromCurrency: string, toCurrency: string, amount: number): Promise<number> {
...
async function main() {
  ...
  if (!(continueConversion)) console.log((chalk.redBright)("Exiting..."))
