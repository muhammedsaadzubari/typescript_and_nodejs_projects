...
const prompts = {
    ...
    promptList: async (message, choices) => {
        let { prompt } = await inquirer.prompt({
            type: "list",
            name: "prompt",
            message,
    ...
    while (true) {
        ...
        while (true) {
            for (let attempts = 5; attempts > 0; attempts--) {
                ...
                let guessedNumber;
                while (true) {
                    guessedNumber = await prompts.promptNumber(`Guess the number! (Between 0 and ${max}). You have only ${attempts} more chances:`);
                ...
                if (guessedNumber === random) {
                    ...
                    score += 5;
                    console.log(chalk.blueBright(`Your score has increased by 5, and now your score is ${score}!`));
                }
                else {
                    console.log(chalk.redBright(`Sorry, the number you guessed '${chalk.yellowBright(guessedNumber)}' is incorrect! Try Again!`));
                }
            }
            console.log(chalk.greenBright(`Your final score is ${chalk.magentaBright(score)}!`));
            if (score >= 20) {
                console.log(chalk.bgCyanBright(chalk.bold(`Congratulations! You have won the game with a score of ${score}/25!`)));
            }
            else {
                console.log(chalk.bgCyanBright(chalk.bold(`You have lost the game with a score of ${score}/25!`)));
            }
