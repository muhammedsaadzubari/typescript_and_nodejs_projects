...
class Animate_Banner {
    #banner = ` Get ready to manage your student data with our Student Management System! \n
                          .----------------.
                          | .------------. |
                          | |   Student  | |
                          | | Management | |
...
class Person {
    ...
    "Phone no";
    "Email";
    constructor(first_name, last_name, age, gender, phone_no, email) {
        this["Age"] = age;
    ...
    #full_name_in_title_case(f_name, l_name) {
        let full_name_in_array = [f_name, l_name];
...
class Student extends Person {
    static no = 0;
    "Roll no." = this.#five_digit_rollno();
    "Enrolled Courses" = [];
    "Balance" = 0;
    "Tution Fee" = 0;
    ...
    constructor(f_name, l_name, age, gender, address, phone_no, email) {
...
class Instructor extends Person {
    "ID" = this.#three_digit_id();
    static no = 0;
    "Salary";
    "Courses Assigned" = [];
    #three_digit_id() {
        return `${++_b.no}`.padStart(3, `0`);
    }
    assign_course(courses) {
        this["Courses Assigned"].push(...courses);
    }
    constructor(f_name, l_name, age, salary, gender, phone_no, email) {
        super(f_name, l_name, age, gender, phone_no, email);
        this["Salary"] = salary;
    }
}
...
class Courses {
    ...
    "Tution Fee";
    #two_digit_id() {
        return `${++_c.no}`.padStart(2, `0`);
    }
    #timing(from, from_am_or_pm, to, to_am_or_pm) {
        return `From ${from}${from_am_or_pm} to ${to}${to_am_or_pm}`;
    }
    constructor(name, timing, tution_fee) {
        this["Name"] = name;
...
class Main {
    ...
    static async #prompt_type_checkbox(message, choices) {
        let { prompt } = await inquirer.prompt({
            name: "prompt",
            type: "checkbox",
            message,
    ...
    static async #add_student() {
        let f_name = await Main.#prompt_type_input("Enter First Name!");
        let l_name = await Main.#prompt_type_input("Enter Last Name!");
        let age = await Main.#prompt_type_number("Enter Age!");
        let gender = await Main.#prompt_type_list("Enter Student's Gender!", ["Male", "Female"]);
        let address = await Main.#prompt_type_input("Enter Residential Address!");
        let phone_no = await Main.#prompt_type_input("Enter Phone no.");
        let email = await Main.#prompt_type_input("Enter email!");
        let student = new Student(f_name, l_name, age, gender, address, phone_no, email);
    ...
    static async #add_instructor() {
        ...
        let l_name = await Main.#prompt_type_input("Enter Last Name!");
        let age = await Main.#prompt_type_number("Enter Age!");
        let gender = await Main.#prompt_type_list("Enter Instructor's Gender!", ["Male", "Female"]);
        let phone_no = await Main.#prompt_type_input("Enter Phone no.");
        let email = await Main.#prompt_type_input("Enter email!");
        let salary = await Main.#prompt_type_number("Enter Instructor's salary!");
        let instructor = new Instructor(f_name, l_name, age, salary, gender, phone_no, email);
    ...
    static async #add_course() {
        let name = await Main.#prompt_type_input("Enter Course' name!");
        console.log("Timing");
        let from = await Main.#prompt_type_number("From:");
        let from_am_or_pm = await Main.#prompt_type_list("<A.M.> Or <P.M.>", ["a.m.", "p.m."]);
        let to = await Main.#prompt_type_number("To:");
        let to_am_or_pm = await Main.#prompt_type_list("<A.M.> Or <P.M.>", [
            "a.m.",
            "p.m.",
        ]);
        let fees = await Main.#prompt_type_number("Enter Course' Fees!");
        let course = new Courses(name, [from, from_am_or_pm, to, to_am_or_pm], fees);
    ...
    static async #edit_student() {
        if (Main.#students.length === 0) {
            ...
            while (bool) {
                ...
                let roll_no = await Main.#prompt_type_input("Enter Roll no. to edit!");
                let index = Main.#students.findIndex((obj) => {
                ...
                if (index !== -1) {
                    ...
                    let address = await Main.#prompt_type_input("Enter Residential Address!");
                    let phone_no = await Main.#prompt_type_input("Enter Phone No.!");
                    let email = await Main.#prompt_type_input("Enter Student's Email!");
                    let student = new Student(f_name, l_name, age, gender, address, phone_no, email);
                    ...
                    console.log(chalk.red("The roll no. you have provide can't be found!"));
    ...
    static async #edit_instructor() {
        if (Main.#instructors.length === 0) {
            ...
            while (bool) {
                ...
                let id = await Main.#prompt_type_input("Enter ID to edit!");
        ...
        if (Main.#courses.length === 0) {
            ...
            while (bool) {
                ...
                if (index !== -1) {
                    ...
                    let from_am_or_pm = await Main.#prompt_type_list("<A.M.> Or <P.M.>", ["a.m.", "p.m."]);
                    let to = await Main.#prompt_type_number("To:");
                    let to_am_or_pm = await Main.#prompt_type_list("<A.M.> Or <P.M.>", ["a.m.", "p.m."]);
                    let fees = await Main.#prompt_type_number("Enter Course' Fees!");
                    let course = new Courses(name, [from, from_am_or_pm, to, to_am_or_pm], fees);
    ...
    static async #delete_student() {
        if (Main.#students.length === 0) {
            ...
            while (bool) {
                console.table(Main.#students);
                let roll_no = await Main.#prompt_type_input("Enter Roll no. to delete!");
    ...
    static async #delete_instructor() {
        if (Main.#instructors.length === 0) {
            console.log(chalk.redBright("You have not any instructor to delete yet!"));
        }
        else {
            let bool = true;
            while (bool) {
                console.table(Main.#instructors);
                let id = await Main.#prompt_type_input("Enter ID to delete!");
    ...
    static async #add_student_balance() {
        if (Main.#students.length === 0) {
            console.log(chalk.redBright("You have not any student to add their balance yet!"));
        }
        else {
            let bool = true;
            while (bool) {
                console.table(Main.#students);
                let roll_no = await Main.#prompt_type_input("Enter Roll no. to add balance!");
    ...
    static async #enroll_student_in_course() {
        if (Main.#students.length === 0) {
            console.log(chalk.redBright("You have not any student to enroll them in a course!"));
        }
        else {
            let bool = true;
            while (bool) {
                console.table(Main.#students);
                let roll_no = await Main.#prompt_type_input("Enter Roll no. to enroll in course!");
                let obj = Main.#students.find((obj) => {
                    return obj["Roll no."] === roll_no;
                });
                if (obj !== undefined) {
                    if (Main.#courses.length === 0) {
                        console.log(chalk.redBright("You have not any course to enroll student!"));
                    }
                    else {
                        let courses_name = await Main.#prompt_type_checkbox("Select courses to enroll student in them!", Main.#courses.map((ele) => ele.Name));
                        let bool = true;
                        while (bool) {
                            if (courses_name.length !== 0) {
                                let courses = courses_name.map((val) => Main.#courses.find((ele) => ele["Name"] === val));
                                obj.enroll_courses(courses);
                                obj.tution_fee(courses);
                                courses.forEach((element) => {
                                    element.add_student(obj);
                                });
                                console.log(chalk.yellow("Student is enrolled in course succesfully!"));
                                console.table(courses);
                                console.table(obj);
                                bool = false;
                            }
                            else {
                                console.log("You have not selected any course to enroll the student!");
                                bool = await Main.#prompt_type_confirm("Do you want to continue!");
                            }
                        }
                    }
    ...
    static async #assign_instructor_to_course() {
        if (Main.#instructors.length === 0) {
            console.log(chalk.redBright("You have not any instructor to assign a course to them!"));
        }
        else {
            let bool = true;
            while (bool) {
                console.table(Main.#instructors);
                let ID = await Main.#prompt_type_input("Enter Instructor's ID. to assign course!");
                let obj = Main.#instructors.find((obj) => {
                    return obj["ID"] === ID;
                });
                if (obj !== undefined) {
                    if (Main.#courses.length === 0) {
                        console.log(chalk.redBright("You have not any course to assign a instructor!"));
                    }
                    else {
                        let courses_name = await Main.#prompt_type_checkbox("Select courses to assign instructor!", Main.#courses.map((ele) => ele.Name));
                        while (bool) {
                            if (courses_name.length !== 0) {
                                let courses = courses_name.map((val) => Main.#courses.find((ele) => ele["Name"] === val));
                                ...
                                console.table(courses);
                                console.log(chalk.yellow("Instructor was assigned to a course succesfully!"));
                                console.table(obj);
                                bool = false;
                            }
                            else {
    ...
    static async #pay_tution_fee() {
        if (Main.#students.length !== 0) {
            if (Main.#students.some((obj) => { obj["Enrolled Courses"].length !== 0; })) {
                ...
                while (bool) {
                    console.table(Main.#students);
                    let rollno = await Main.#prompt_type_input("Enter student's roll no. to pay tution fee!");
                    let obj = Main.#students.find((obj) => {
    ...
    static async #add() {
        let operation = await Main.#prompt_type_list("Which operation do you want to perform", ["Add Student", "Add Instructor", "Add Course", "Exit"]);
        switch (operation) {
            case "Add Student":
    ...
    static async #view() {
        let operation = await Main.#prompt_type_list("Which operation do you want to perform", ["View Student", "View Instructor", "View Course", "Exit"]);
        switch (operation) {
            case "View Student":
                console.table(Main.#students);
                break;
    ...
    static async #edit() {
        let operation = await Main.#prompt_type_list("Which operation do you want to perform", ["Edit Student", "Edit Instructor", "Edit Course", "Exit"]);
        switch (operation) {
            case "Edit Student":
    ...
    static async #delete() {
        let operation = await Main.#prompt_type_list("Which operation do you want to perform", ["Delete Student", "Delete Instructor", "Delete Course", "Exit"]);
        switch (operation) {
            case "Delete Student":
    ...
    static async #payment_balance() {
        let operation = await Main.#prompt_type_list("Which operation do you want to perform", ["Pay Student's tution fee", "Add student balance", "Exit"]);
        switch (operation) {
    ...
    static async #enrollement_assignment() {
        let operation = await Main.#prompt_type_list("Which operation do you want to perform", ["Enroll Student in a course", "Assign Instructor to course", "Exit"]);
        switch (operation) {
    ...
    static async main() {
        ...
        let bool = true;
        while (bool) {
            let operation = await Main.#prompt_type_list("Which operation do you want to perform!", [
                "Add",
                "Edit",
                "View",
                "Delete",
            ...
            bool = await Main.#prompt_type_confirm("Do you want to move on main menu again!");
        }
    }
}
Main.main();
